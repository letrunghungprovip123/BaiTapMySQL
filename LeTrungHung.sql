...
INSERT INTO orders (
	food_id, user_id, amount, arr_sub_id)
		VALUES(
			1, 1, 2, 'SUB123, SUB456'), (
			2, 2, 1, 'SUB789'), (
			3, 3, 3, 'SUB101, SUB102'), (
...
WHERE
	...
	FROM(
		SELECT
			COUNT(lr.user_id) AS "Số Like", u.user_id FROM users u
...
WHERE
	res_id IN(
		SELECT
			res_id FROM (
				SELECT
					COUNT(lr.res_id) AS "Số Like", r.res_id FROM restaurant r
...
WHERE
	user_id IN(
		SELECT
			user_id FROM (
				SELECT
					COUNT(o.user_id) AS "Số order", o.user_id FROM users u
	...
	user_id IN(
		SELECT
			...
			WHERE
				...
				AND rr.rate_res_id IS NULL) AS baitap4);